...
class RoleDispatcher:
    ...
    def should_speak(
        ...
        Args:
            role_config: 角色配置，包含 activity_level, trigger_mode, keywords, min_interval
            message_count_since_last: 距上次发言的消息条数
    ...
    def select_triggered_role(
        ...
        Args:
            message_text: 输入消息文本
            roles_config: 角色配置字典，key为角色类型，value为包含keywords等的配置

        Returns:
            匹配的角色类型key，无匹配返回None
        """
        if not message_text:
            return None
    ...
    def round_robin_next(
        ...
        Args:
            last_speaker_index: 上次发言角色在有序列表中的索引
            total_roles: 参与轮序的角色总数
